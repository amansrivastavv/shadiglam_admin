  ...
  return (
    <header className="h-20 flex justify-between items-center px-6 md:px-10 bg-white/80 backdrop-blur-xl sticky top-0 z-40 border-b border-slate-100">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        <button className="p-2 text-slate-400 hover:text-brand-blue hover:bg-slate-50 rounded-full transition-all">
          <Search size={20} />
        </button>
        <button className="p-2 text-slate-400 hover:text-brand-blue hover:bg-slate-50 rounded-full transition-all relative">
          <Bell size={20} />
          <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>
        </button>
        
        <div className="relative" onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
          <div className="flex items-center gap-2 md:gap-3 p-1 rounded-full hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition-all group">
            <div className="w-8 h-8 md:w-9 md:h-9 bg-brand-blue text-white rounded-full flex items-center justify-center font-bold text-xs md:text-sm shadow-md group-hover:scale-105 transition-transform">AS</div>
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                <button className="w-full flex items-center gap-3 p-3 rounded-xl text-sm text-slate-600 hover:bg-slate-50 hover:text-brand-blue transition-all font-medium">
